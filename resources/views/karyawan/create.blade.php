...
@section('title', 'Tambah Karyawan - Aplikasi Penggajian')

@section('content')
    <x-sidebar active="karyawan.create" />
        ...
        <div class="max-w-5xl px-4 py-10 lg:ml-80">
            <div class="mb-8 rounded-xl border border-slate-200 bg-white p-8 shadow-[0_35px_80px_rgba(15,23,42,0.08)]">
                <div class="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
                    <div class="space-y-3">
                        ...
                        <h1 class="mt-3 text-3xl font-semibold tracking-tight text-slate-900">Tambah Karyawan Baru</h1>
                        <p class="mt-2 max-w-2xl text-sm leading-6 text-slate-600">Tambahkan nama karyawan agar bisa membuat
                            slip gaji di modul terpisah.</p>
                    </div>
            ...
            <form action="{{ route('karyawan.store') }}" method="post"
                            ...
                            <select id="jabatan" name="jabatan" required
                                ...
                                @foreach ($jabatanOptions as $jabatan)
                                    <option value="{{ $jabatan->value }}"
                                        {{ old('jabatan') === $jabatan->value ? 'selected' : '' }}>
                ...
                <div class="mt-8 flex justify-end">
                    <button type="submit"
                        class="inline-flex items-center justify-center rounded-full bg-indigo-600 px-8 py-4 text-sm font-semibold text-white shadow-lg shadow-indigo-500/20 transition hover:bg-indigo-700">Simpan
                        Karyawan</button>
                </div>
            </form>
        </div>
